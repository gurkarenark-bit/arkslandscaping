...
test('magic link is reusable until expiry and invalid after expiry', async () => {
  ...
  assert.equal(isMagicLinkValid(record, token), true, 'same link should remain reusable before expiry');

  const expiredAt = new Date(Date.now() - 1_000).toISOString();
  assert.equal(isMagicLinkValid({ ...record, expires_at: expiredAt }, token), false);
});
